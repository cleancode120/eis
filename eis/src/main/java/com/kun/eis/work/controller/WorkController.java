package com.kun.eis.work.controller;
...
import com.kun.eis.work.service.WorkService;
...
public class WorkController {
	...
	public String testHome(@ModelAttribute("searchVO") WorkVO vo, Model model, HttpSession session) throws Exception {	
	...
	public String testList(@ModelAttribute("searchVO") WorkVO vo, Model model, HttpSession session) throws Exception {	
		
		// 게시판 UTIL
		BoardUtil boardUtil = new BoardUtil();
		List<WorkVO> testList = null;
		...
		try {
    	    ...
    	    */
			...
			// 전체 검색 결과
			testList = workService.testList(vo);
	...
	@ResponseBody
	public Object procEqpmntManage(@ModelAttribute("vo") WorkVO vo, Model model, HttpSession session) throws Exception {	
		...
		try {
			...
			workVO = workService.selectPwCode(vo);
			rstFlag = true;
