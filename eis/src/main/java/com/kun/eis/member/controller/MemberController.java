...
public class MemberController {
	...
	public String testHome(@ModelAttribute("searchVO") MemberVO vo, Model model, HttpSession session) throws Exception {	
	...
	public String testList(@ModelAttribute("searchVO") MemberVO vo, Model model, HttpSession session) throws Exception {	
		
		// 게시판 UTIL
		BoardUtil boardUtil = new BoardUtil();
		List<MemberVO> testList = null;
		...
		try {
    	    ...
    	    */
			...
			// 전체 검색 결과
			testList = memberService.testList(vo);
	...
	@ResponseBody
	public Object procEqpmntManage(@ModelAttribute("vo") MemberVO vo, Model model, HttpSession session) throws Exception {	
		...
		try {
			...
			memberVO = memberService.selectPwCode(vo);
			rstFlag = true;
